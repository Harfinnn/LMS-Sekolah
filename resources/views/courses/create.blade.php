...
<div class="p-4 sm:ml-64 mt-16">
    <div class="max-w-2xl mx-auto bg-slate-900 rounded-2xl shadow p-6 text-slate-100">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-extrabold text-green-400">Tambah Course</h1>
                <p class="text-slate-400 text-sm mt-1">
                    Course di sini adalah mata pelajaran untuk satu kelas, misalnya "Matematika Kelas X A".
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    ...
                    <select name="grade_level"
                        class="w-full bg-slate-800 p-2 rounded border border-slate-700 focus:outline-none focus:ring-2 focus:ring-green-500">
                        <option value="" disabled {{ old('grade_level', $grade ?? '') ? '' : 'selected' }}>Pilih Tingkat</option>
                        @foreach(['X','XI','XII'] as $g)
                        <option value="{{ $g }}" {{ old('grade_level', $grade ?? '') == $g ? 'selected' : '' }}>
                            {{ $g }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="class_group"
                        class="w-full bg-slate-800 p-2 rounded border border-slate-700 focus:outline-none focus:ring-2 focus:ring-green-500">
                        <option value="" disabled {{ old('class_group', $class ?? '') ? '' : 'selected' }}>Pilih Sub Kelas</option>
                        @foreach(['A','B','C'] as $c)
                        <option value="{{ $c }}" {{ old('class_group', $class ?? '') == $c ? 'selected' : '' }}>
                            {{ $c }}
                        </option>
                        @endforeach
                    </select>
